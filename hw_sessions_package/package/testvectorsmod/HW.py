...
# def MontExp_MontPowerLadder(X, E, N):
#     # Returns (X^E) mod N, and also RN and R2N for debugging
...
#             A       = MontMul(A, A, N)
#     A = MontMul(A, 1, N)
#     return A, RN, R2N  # 返回 A、RN 和 R2N

def MontExp_MontPowerLadder(X, E, N):
    # Returns (X^E) mod N, and also prints A and X_tilde at each iteration
    ...
    print("Initial X_tilde  = ", hex(X_tilde))
    for i in range(0, t):
        bit = helpers.bit(E, t - i - 1)
        print(f"\nIteration {i+1}/{t}, E[{t - i - 1}] = {bit}")
        if bit == 1:
            A_prev = A
            X_tilde_prev = X_tilde
            A       = MontMul(A, X_tilde, N)
            X_tilde = MontMul(X_tilde, X_tilde, N)
            print("A = MontMul(A_prev, X_tilde_prev, N)")
            print("X_tilde = MontMul(X_tilde_prev, X_tilde_prev, N)")
        else:
            A_prev = A
            X_tilde_prev = X_tilde
            X_tilde = MontMul(A, X_tilde, N)
            A       = MontMul(A, A, N)
            print("X_tilde = MontMul(A_prev, X_tilde_prev, N)")
            print("A = MontMul(A_prev, A_prev, N)")
        print("Updated A        = ", hex(A))
        print("Updated X_tilde  = ", hex(X_tilde))
    A = MontMul(A, 1, N)
    print("\nFinal A after MontMul with 1:")
    print("A                = ", hex(A))
    return A, RN, R2N
